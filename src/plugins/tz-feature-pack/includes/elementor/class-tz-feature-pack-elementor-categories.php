...
class TZ_Categories_Grid extends Widget_Base {
	...
	protected function render() {
		...
		if ( 'carousel' == $settings['categories_output'] ) {
			$carousel_data = ' with-slider" data-owl="container" data-owl-type="content-carousel" data-owl-slides="' . esc_attr($settings['columns_number']) . '" data-owl-custom-nav="' . esc_attr($settings['show_arrows']) . '" data-owl-arrows="no" data-owl-dots="' . esc_attr($settings['show_dots']) . '" data-owl-autoplay="' . esc_attr($settings['autoplay']) . '" data-owl-margin="' . esc_attr($settings['slide_gap']) . '"';
		}
		...
		if ( is_array($settings['button_url']) && !empty($settings['button_url']) ) {
			...
			if ($label && $label!='') {
				$output .= '<a class="button category-grid-button" href="' . esc_attr($url) . '"' . $target . $nofollow .'>' . esc_html($label) . '</a>';
			}
		}
		/* Carousel navigation */
		...
		if ( is_array($settings['chosen_cats']) && !empty($settings['chosen_cats']) ) {
			...
			foreach ( $settings['chosen_cats'] as $id) {
				...
				if ( $term ) {
					...
					if ( $image ) {
						$output .= '<div class="img-wrapper"><a href="'.get_term_link($term).'">' . $image . '</a></div>';
					}
					...
					if ( $settings['sub_cat_number'] > 0 ) {
                        ...
                        foreach ($subcats as $index => $sc) {
                          ...
                          $output .= '<li><a href="' . esc_attr($link) . '">' . esc_html($sc->name) . '</a></li>';
