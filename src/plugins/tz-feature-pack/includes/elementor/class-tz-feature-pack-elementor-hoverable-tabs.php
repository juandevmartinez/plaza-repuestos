      ...
      [
		...
		$repeater->add_control(
			...
			[
				...
				'type' => Controls_Manager::TEXT,
				'label_block' => true,
				'default' => esc_html__( 'Tab Title', 'tz-feature-pack' ),
			]
		);
    ...
    $css_class = 'tz-hoverable-tabs';
		...
		if ( $settings['tabs'] ) {
			...
			foreach ( $settings['tabs'] as $tab ) {
				...
				$tab_title_setting_key = $this->get_repeater_setting_key( 'tab_title', 'tabs', $tab_index );

				/* Tabs nav links */
				$tabs_nav_output .= '<li>';
				$tabs_nav_output .= esc_html($tab['tab_title']);
				/* Tabs content */
