    ...
    $this->add_control(
      ...
      [
		...
		$this->add_control(
			'loop',
			[
				'label' => esc_html__( 'Loop Items', 'tz-feature-pack' ),
        ...
        $this->add_control(
            'columns_mobile',
            [
                'label' => esc_html__( 'Number of products per row on mobile', 'tz-feature-pack' ),
                'type' => Controls_Manager::SELECT,
                'default' => '1',
                'options' => [
                    '1' => esc_html__( '1 Col', 'tz-feature-pack' ),
                    '2' => esc_html__( '2 Cols', 'tz-feature-pack' ),
                ],
            ]
        );
		...
		$this->add_control(
			...
			[
	          ...
	          'description' => esc_html__( 'Comma separated list of category slugs which products you want to output. For shortcode "Products by category".', 'tz-feature-pack' ),
                    ...
                    [
                        'name' => 'slideby',
                        'label'   => esc_html__( 'Number of Products to slide', 'tz-feature-pack' ),
                        'type'    => Controls_Manager::NUMBER,
                        'default' => 1,
                        'min'     => 1,
                        'max'     => 5,
      ...
      foreach ( $settings['product_shortcodes'] as $product_shortcode ) {
        ...
        if ( $product_shortcode === reset($settings['product_shortcodes']) ) {
          ...
          $tabs_nav_output .= '<li role="presentation" class="active"><a href="#section-'.esc_attr($nav_arg).'" data-toggle="tab" rel="tab" title="'.esc_attr($product_shortcode['tab_title']).'">';
					...
					$setting_loop = ( isset( $settings['loop'] ) && (  $settings['loop'] == 'yes' ) )  ? 'true' : 'false';

                    $slide_by =  isset( $product_shortcode['slideby'] )  ? $product_shortcode['slideby'] : 1;

					$tabs_content_output .= '<div role="tabpanel" class="tab-pane fade in active row" id="section-'.esc_attr($nav_arg).'" data-owl="container" data-owl-margin="' . esc_attr($settings['slide_gap']) . '" data-owl-slides="' . esc_attr($settings['columns_number']) . '"'.esc_html($owl_row_data).' data-owl-loop="'.esc_attr( $setting_loop ).'" data-owl-col-mobile="'.esc_attr($mobile_cols).'" data-owl-slide-by="'.esc_attr($slide_by).'">';
					...
					$tabs_content_output .= '<div role="tabpanel" class="tab-pane fade row" id="section-'.esc_attr($nav_arg).'" data-owl="container" data-owl-margin="' . esc_attr($settings['slide_gap']) . '" data-owl-slides="' . esc_attr($settings['columns_number']) . '"'.esc_html($owl_row_data).' data-owl-loop="'.esc_attr( $setting_loop ).'" data-owl-col-mobile="'.esc_attr($mobile_cols).'" data-owl-slide-by="'.esc_attr($slide_by).'">';
					$tabs_content_output .= '<span class="carousel-loader"></span>';
					$tabs_content_output .= '<div class="slider-navi"><span class="prev"></span><span class="next"></span></div>';
					$tabs_content_output .= $shortcode;
					$tabs_content_output .= '</div>';
