...
var couponController = ( function ( $ ) {
    var privateApi = {
        addEventHandlers: function addEventHandlers() {
            $( '#wcmp-afm-add-coupon' )
                .on( 'click', '#wcmp_afm_coupon_submit, #wcmp_afm_coupon_draft', this.saveCoupon )
            ;
            return false;
        },
        ...
        saveCoupon: function saveCoupon() {
            $( 'form#wcmp-afm-add-coupon' ).trigger( 'before_coupon_save' );
            var status = ( this.id === 'wcmp_afm_coupon_submit' ) ? 'publish' : ( this.id === 'wcmp_afm_coupon_draft' ) ? 'draft' : '';
