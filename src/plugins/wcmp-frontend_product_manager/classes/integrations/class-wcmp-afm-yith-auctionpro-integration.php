...
class WCMp_AFM_Yith_Auctionpro_Integration {
    ...
    function delete_auction_bid_data() {
        $product_id = isset($_REQUEST['auction_id']) ? (int) $_REQUEST['auction_id'] : 0;
        $user_id = isset($_REQUEST['user_id']) ? (int) $_REQUEST['user_id'] : 0;
        $date = isset($_REQUEST['datetime']) ? (int) $_REQUEST['datetime'] : 0;
        $datetime = date( 'Y-m-d H:i:s', $date );
        $products_url = wcmp_get_vendor_dashboard_endpoint_url(get_wcmp_vendor_settings('wcmp_products_endpoint', 'vendor', 'general', 'edit-product'), $product_id);
        $wpnonce = isset($_REQUEST['_wpnonce']) ? $_REQUEST['_wpnonce'] : '';
        $vendor = get_wcmp_product_vendors($product_id);
        $current_user_ids = array( get_current_user_id() );
        if ($wpnonce && wp_verify_nonce($wpnonce, 'yith_auction_bid_delete') && $product_id && in_array($vendor->id, $current_user_ids )) {
            $bids = YITH_Auctions()->bids;
            $bids->delete_customer_bid($product_id, $user_id, $datetime);
            wc_add_notice(__('Customer Bid Deleted!', 'dc-woocommerce-multi-vendor'), 'success');
    ...
    public function save_auction_meta( $product_id, $data ) {
        ...
        if ( ! $exist_auctions ) {
            yit_save_prop($auction_product, '_price',$_POST['_yith_auction_start_price']);
        }

        // save all data
        foreach ( $data as $key => $value ) {
            if ( substr( $key, 0, 6 ) === "_yith_" ) {
                if($key == '_yith_auction_for' || $key == '_yith_auction_to') {
                    update_post_meta( $product_id, $key, strtotime( get_gmt_from_date( $value ) ) );
                } else {
