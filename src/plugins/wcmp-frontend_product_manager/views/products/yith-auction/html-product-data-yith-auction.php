 ...
 *
 * This template can be overridden by copying it to yourtheme/wcmp-afm/products/yith-auction/html-product-data-yitch-auction.php.
 *
...
<div role="tabpanel" class="tab-pane fade" id="<?php esc_attr_e( $tab ); ?>">
    <div class="row-padding">
        ...
        <?php
        $from_auction = ($datetime = $auctionable_product->get_start_date()) ? absint($datetime) : '';
        $from_auction = $from_auction ? get_date_from_gmt(date('Y-m-d H:i:s', $from_auction)) : '';
        $to_auction = ($datetime = $auctionable_product->get_end_date()) ? absint($datetime) : '';
