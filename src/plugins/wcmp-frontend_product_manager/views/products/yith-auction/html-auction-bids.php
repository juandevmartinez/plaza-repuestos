            ...
            <tbody>
                ...
                foreach ( $auction_list as $object => $auction_obj ) {
                    ...
                    <tr class="yith-wcact-row">
                        ...
                        <td>
                            <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(array('auction_id' => $auction_obj->auction_id, 'user_id' => $auction_obj->user_id, 'datetime' => strtotime($auction_obj->date)), wcmp_get_vendor_dashboard_endpoint_url(get_wcmp_vendor_settings('wcmp_products_endpoint', 'vendor', 'general', 'edit-product'), $auction_obj->auction_id)), 'yith_auction_bid_delete')); ?>" class="yith-wcact-delete-bid" data-user-id="<?php echo absint( ( $auction_obj->user_id ) ) ?>" data-date-time="<?php echo $auction_obj->date ?>" data-product-id="<?php echo $id ?>"><?php esc_html_e( 'Delete bid', 'yith-auctions-for-woocommerce' ); ?></a>
                        </td>
                    </tr>
                    <?php
                }
