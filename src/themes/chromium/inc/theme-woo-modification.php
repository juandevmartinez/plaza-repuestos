...
if (!function_exists('chromium_product_extra_class')) {
	function chromium_product_extra_class( $classes ) {
		...
		if ( is_woocommerce() ) {
			$classes[] = 'mobile-'.get_theme_mod( 'mobile_products_qty', 'one' ).'-col-products';
		}
...
if ( ! function_exists('chromium_output_custom_labels' ) ) {
	function chromium_output_custom_labels() {
		...
		if ( ( is_array($post_terms) && count( $post_terms ) ) || ( is_array( $default_terms ) && count( $default_terms ) ) ) {
		    if ( ( is_array($default_terms) || $default_terms instanceof Countable ) && count($default_terms) )
...
if ( class_exists('WCV_Vendor_Shop') ) {
    remove_action('woocommerce_after_shop_loop_item', [ 'WCV_Vendor_Shop', 'template_loop_sold_by' ], 9);
    add_action( 'woocommerce_shop_loop_item_title', [ 'WCV_Vendor_Shop', 'template_loop_sold_by' ], 2 );
}
...
add_action( 'woocommerce_before_checkout_form_cart_notices', 'chromium_return_to_cart_notice_button' );
function chromium_return_to_cart_notice_button(){

    $message = esc_html__('Go back to the Cart page', 'chromium');
    $button_text = esc_html__('Back to Cart', 'chromium');
